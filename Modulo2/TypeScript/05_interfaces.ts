...
function validarUsuario(u: Usuario): boolean {
  return u.nombre.length > 0 && u.email.indexOf("@") !== -1;
}
...
// anaGarcia.id = 99;  // ❌ Error — id es readonly

console.log(`${anaGarcia.nombre} — tel: ${anaGarcia.telefono ?? "no registrado"}`);
