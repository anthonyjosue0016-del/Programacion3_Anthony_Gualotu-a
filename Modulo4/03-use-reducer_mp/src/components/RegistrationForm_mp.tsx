...
export default function RegistrationForm() {
  ...
  function validate(): boolean {
    ...
    if (!emailRegex.test(state.email.trim())) errors.email = 'Email invÃ¡lido'
    if (state.password.length < 6) errors.password = 'MÃ­nimo 6 caracteres'
    ...
    >
      ...
      {state.status === 'error' && (
        <div style={{ padding: 12, background: '#fee2e2', borderRadius: 6, color: '#991b1b' }}>
          âš ï¸ OcurriÃ³ un error al enviar el formulario
        </div>
      )}
