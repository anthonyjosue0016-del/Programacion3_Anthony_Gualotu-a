...
export default function TaskManager() {
  ...
  // AGREGAR â€” spread del array anterior mÃ¡s el nuevo item
  function addTask() {
    if (!input.trim()) return

    const taskTitle = title.trim() || 'Nueva tarea'

    setTasks((prev) => [
      ...prev,
      { id: Date.now(), title: taskTitle, text: input.trim(), done: false },
    ])
    setInput('')
    setTitle('')
  }

  // ELIMINAR â€” filter crea un nuevo array sin el elemento
  function removeTask(id: number) {
    setTasks((prev) => prev.filter((task) => task.id !== id))
  }

  // ACTUALIZAR â€” map crea un nuevo array con el elemento modificado
  ...
  return (
    <div style={{ maxWidth: 380 }}>
      ...
      {tasks.length === 0 && (
        <p style={{ color: '#999', fontSize: 14 }}>No hay tareas. Â¡Agrega una!</p>
      )}
